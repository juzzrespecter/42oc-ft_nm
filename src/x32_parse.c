...
/**
 * Raw ELF symbol into nm output formatted symbol, filtered by visibility config.
 * in nm flags.
 * @param wr_sym ;; raw symbol wrapper
 * @param bin    ;; binary context
 * @param ctx    ;; nm context
 * @return       ;; nm sym, or NULL if not printable
 */
